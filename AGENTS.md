...
- `neural-player.js` — `NeuralPlayer`: owns the `<audio>` element for natural voices. Generates one sentence at a time from the playhead, stitches ready sentences into padded WAV windows with pauses baked in, hands off to the next window on `timeupdate` (not `ended`, which iOS blocks when locked), and plays a silent loop while waiting. Needs ≥3 s of contiguous audio before (re)starting.
- `neural-segments.js` — sentence-level synthesis segments (≤220 chars) with structural pauses; `neural-audio.js` — PCM trim, WAV encode/decode, stitching.
- `tts-worker.js` — worker for neural backends: FIFO of single sentences, returns silence-trimmed 16-bit PCM + metrics.
- `kokoro-runtime.js` — Kokoro on plain `onnxruntime-web/wasm` (a faithful port of kokoro-js 1.2.1's frontend). transformers.js' JSEP/asyncify ONNX builds crash on iOS 26.2+, so kokoro-js is only used for WebGPU. `model-fetch.js` — shared progress + CacheStorage downloads.
...
- `tts-backends.js` — `KokoroWebGPU`/`KokoroWasm`/`KittenWasm` worker backends (`synthesize()`), `WorkerTtsBackend` lifecycle, and cached WebGPU probe (`probeKokoroWebGpu` only after explicit Kokoro+WebGPU pick).
...
- Make granular generating states visible: file + percent + backend (e.g. `Downloading onnx/model.onnx [kitten-wasm] · 43%`), `Generating segment 3/12 [kokoro-webgpu]`, `Synthesizing …`, with `setNeuralLoading` driving the player's progress rule and the voice sheet's `#engine-status` (no blocking overlay for model downloads). Never leave the player permanently stuck in `buffering`.
...
- Preserve play, pause, resume, seeking, ±15-second skips, saved position, playback rate, metadata, and lock-screen/control-center actions. Seeking only changes which sentence the scheduler generates next; finished sentences are kept. A sentence that fails twice becomes a short silence instead of ending playback.
